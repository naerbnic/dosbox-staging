...
public:
	...
	virtual bool CheckShouldBreakOnInt(PhysPt adr, uint8_t intNr,
	                                   uint16_t ahValue, uint16_t alValue) = 0;
...
public:
	...
	{
		return segment == seg && segment == off;
	}

	bool CheckShouldBreak(Bitu seg, Bitu off) override
	{
		return location == GetAddress(seg, off);
	}
...
public:
	...
	{
		if (this->intNr == intNr) {
			return (((this->ahValue == BPINT_ALL) ||
			         (this->ahValue == ahValue)) &&
			        ((this->alValue == BPINT_ALL) ||
	...
	void Show(int nr) override
	{
		if (ahValue == BPINT_ALL) {
			DEBUG_ShowMsg("%02X. BPINT %02X\n", nr, intNr);
		} else if (alValue == BPINT_ALL) {
...
public:
	...
	bool CheckShouldBreak(Bitu seg, Bitu off) override
	{
		...
		if (prevValue != value) {
			...
			DEBUG_ShowMsg("DEBUG: Memory breakpoint %s: %s - %02X -> %02X\n",
			              GetAddressTypeForDebug(),
			              GetTextAddress().c_str(),
			              prevValue,
			              value);
			prevValue = value;
			return true;
...
protected:
	...
	virtual std::string GetTextAddress() const = 0;
...
protected:
	...
	{
		return GetAddress(segment, offset);
	}

	std::string GetTextAddress() const override {
		return format_string("%04X:%04X", segment, offset);
	}
...
protected:
	...
	std::string GetTextAddress() const override {
		return format_string("%08X", location);
	}
...
public:
	...
	bool CheckShouldBreak(Bitu seg, Bitu off) override
	{
		...
		Descriptor desc;
		if (!cpu.gdt.GetDescriptor(segment, desc)) {
			return false;
		}
		if (desc.GetLimit() == 0) {
			return false;
...
protected:
	...
	std::string GetTextAddress() const override {
		return format_string("%04X:%08X", segment, offset);
	}
...
public:
	...
	void NotifyMemoryRead(const PhysPt addr, std::size_t mem_size) override
	{
		const PhysPt location_begin = location;
...
class BreakpointSet {
private:
	using BreakpointList = std::list<std::unique_ptr<CBreakpoint>>;
	using BreakpointIter = typename BreakpointList::iterator;
...
public:
	...
	void ActivateBreakpointsExceptAt(PhysPt adr)
	{
	...
	bool DeleteByIndex(uint16_t index)
	{
		...
		std::unique_ptr<CBreakpoint> bp = std::move(*it);
	...
	bool CheckBreakpoint(Bitu seg, Bitu off)
	{
	...
	bool IsBreakpoint(uint16_t seg, uint32_t off)
	{
		return FindPhysBreakpoint(seg, off, false) != breakpoints_.end();
	}

	bool IsTempBreakpoint(uint16_t seg, uint32_t off)
	{
		return FindPhysBreakpoint(seg, off, true) != breakpoints_.end();
	}
...
private:
	BreakpointIter FindPhysBreakpoint(uint16_t seg, uint32_t off, bool once)
	{
		if (breakpoints_.empty()) {
...
void DEBUG_UpdateMemoryReadBreakpoints(const PhysPt addr)
{
	...
	BPoints.UpdateMemoryReadBreakpoints(addr, sizeof(T));
}
...
bool DEBUG_Breakpoint(void)
{
	...
	if (!BPoints.CheckBreakpoint(SegValue(cs), reg_eip)) {
		return false;
	}
	// Found. Breakpoint is valid
	// PhysPt where=GetAddress(SegValue(cs),reg_eip); -- "where" is unused
	BPoints.DeactivateBreakpoints(); // Deactivate all breakpoints
	return true;
}
...
bool DEBUG_IntBreakpoint(uint8_t intNum)
{
	...
	if (!BPoints.CheckIntBreakpoint(where, intNum, reg_ah, reg_al)) {
		return false;
	}
	// Found. Breakpoint is valid
	BPoints.DeactivateBreakpoints(); // Deactivate all breakpoints
...
static bool StepOver()
{
	...
	if (strstr(dline, "call") || strstr(dline, "int") ||
		...
		if (!BPoints.IsTempBreakpoint(SegValue(cs), reg_eip + size)) {
			BPoints.AddBreakpoint(SegValue(cs), reg_eip + size, true);
		}
		debugging = false;
		DrawCode();
		return true;
...
bool ParseCommand(char* str)
{
	...
	if (command == "BP") { // Add new breakpoint
		...
		found++; // skip ":"
		uint32_t ofs = GetHexValue(found, found);
		BPoints.AddBreakpoint(seg, ofs, false);
	...
	if (command == "BPM") { // Add new breakpoint
		...
		found++; // skip ":"
		uint32_t ofs = GetHexValue(found, found);
		BPoints.AddMemBreakpoint(seg, ofs);
	...
	if (command == "BPINT") { // Add Interrupt Breakpoint
		...
		if ((valAH == 0x00) && (*found == '*' || all)) {
			BPoints.AddIntBreakpoint(intNr, BPINT_ALL, BPINT_ALL, false);
			...
			if ((valAL == 0x00) && (*found == '*' || all)) {
				...
				BPoints.AddIntBreakpoint(intNr, valAH, valAL, false);
	...
	if (command == "logcode") { // Shared code between all logs
		...
		debugging = false;
		BPoints.ActivateBreakpointsExceptAt(SegPhys(cs) + reg_eip);
	...
	if (command == "INT") { // start int.
		...
		BPoints.AddBreakpoint(SegValue(cs), reg_eip, true);
		BPoints.ActivateBreakpointsExceptAt(SegPhys(cs) + reg_eip - 1);
		debugging = false;
		DrawCode();
		DOSBOX_SetNormalLoop();
		CPU_HW_Interrupt(intNr);
...
Bitu DEBUG_Loop(void)
{
	...
	if ((oldCS != SegValue(cs)) || (oldEIP != reg_eip)) {
		BPoints.AddBreakpoint(oldCS, oldEIP, true);
		BPoints.ActivateBreakpointsExceptAt(SegPhys(cs) + reg_eip);
		debugging = false;
		DOSBOX_SetNormalLoop();
		return 0;
...
static void LogMCBChain(uint16_t mcb_segment)
{
	...
	uint16_t DOS_dataOfs = static_cast<uint16_t>(dataOfs); // Realmode
	                                                       // addressing only
...
void DEBUG_CheckExecuteBreakpoint(uint16_t seg, uint32_t off)
{
	if (pDebugcom && pDebugcom->IsActive()) {
		BPoints.AddBreakpoint(seg, off, true);
		BPoints.ActivateBreakpointsExceptAt(SegPhys(cs) + reg_eip);
		pDebugcom = nullptr;
	}
}
